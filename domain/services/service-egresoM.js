...
const ormEgresoM = require('../orm/orm-egresoM');

exports.GetAll = async (req, res) =>{
    let status = true, errorCode ='', message='', data='', statusCode=0, resp={};
    try{
        respOrm = await ormEgresoM.GetAll();
        if(respOrm.err){
...
exports.GetById = async (req, res) =>{
    ...
    try{
        ...
        respOrm = await ormEgresoM.GetById(id);
...
exports.GetByUsu = async (req, res) =>{
    ...
    try{
        const id = req.params.id;
        respOrm = await ormEgresoM.GetByUsu(id);
        if(respOrm && respOrm.err){
            status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
        }else{
            if (respOrm) {
...
exports.GetByDate = async (req, res) =>{
    ...
    try{
        ...
        respOrm = await ormEgresoM.GetByDate(Dateini, Datefin);
        if(respOrm && respOrm.err){
            status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
        }else{
            if (respOrm) {
...
exports.Store = async (req, res) =>{
    ...
    try{
        ...
        if( egresos ){
            respOrm = await ormEgresoM.Store( egresos );
            if(respOrm.err){
                status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
            }else{
