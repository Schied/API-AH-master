...
const ormMaterial = require('../orm/orm-material');

exports.GetAll = async (req, res) =>{
    let status = true, errorCode ='', message='', data='', statusCode=0, resp={};
    try{
        respOrm = await ormMaterial.GetAll();
        if(respOrm.err){
...
exports.GetById = async (req, res) =>{
    ...
    try{
        ...
        respOrm = await ormMaterial.GetById(id);
...
exports.GetByFilter = async (req, res) =>{
    let status = true, errorCode ='', message='', data='', statusCode=0, resp={};
    try{
        const { codigo_mat, nombre_mat, color_mat, unidad_mat } = req.body;
        respOrm = await ormMaterial.GetByFilter( codigo_mat, nombre_mat, color_mat, unidad_mat );
        if(respOrm.err){
            status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
        }else{
            message = 'Success Response', data = respOrm, statusCode = data.length > 0 ? enum_.CODE_OK : enum_.CODE_NO_CONTENT;
...
exports.GetFilters = async (req, res) =>{
    let status = true, errorCode ='', message='', data='', statusCode=0, resp={};
    try{
        respOrm = await ormMaterial.getFilters();
        if(respOrm.err){
            status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
        }else{
            message = 'Success Response', data = respOrm, statusCode = enum_.CODE_OK;
...
exports.Store = async (req, res) =>{
    let status = true, errorCode ='', message='', data='', statusCode=0, resp={};
    try{
        const { codigo_mat, nombre_mat, color_mat, unidad_mat, cantidad_mat } = req.body;
        if( codigo_mat && nombre_mat && color_mat && unidad_mat && cantidad_mat ){
            respOrm = await ormMaterial.Store( codigo_mat, nombre_mat, color_mat, unidad_mat, cantidad_mat );
            if(respOrm.err){
                status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
            }else{
