...
const ormIngresoM = require('../orm/orm-ingresoM');

exports.GetAll = async (req, res) =>{
    let status = true, errorCode ='', message='', data='', statusCode=0, resp={};
    try{
        respOrm = await ormIngresoM.GetAll();
        if(respOrm.err){
...
exports.GetById = async (req, res) =>{
    ...
    try{
        ...
        respOrm = await ormIngresoM.GetById(id);
...
exports.GetByUsu = async (req, res) =>{
    ...
    try{
        const id = req.params.id;
        respOrm = await ormIngresoM.GetByUsu(id);
        if(respOrm && respOrm.err){
            status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
        }else{
            if (respOrm) {
...
exports.GetByDate = async (req, res) =>{
    ...
    try{
        ...
        respOrm = await ormIngresoM.GetByDate(Dateini, Datefin);
        if(respOrm && respOrm.err){
            status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
        }else{
            if (respOrm) {
...
exports.Store = async (req, res) =>{
    ...
    try{
        ...
        if( ingresos ){
            respOrm = await ormIngresoM.Store( ingresos );
            if(respOrm.err){
                status = false, errorCode = respOrm.err.code, message = respOrm.err.messsage, statusCode = enum_.CODE_BAD_REQUEST;
            }else{
